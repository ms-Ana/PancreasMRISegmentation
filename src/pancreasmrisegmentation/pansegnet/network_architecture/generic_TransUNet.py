...
from pancreasmrisegmentation.pansegnet.network_architecture.custom_modules.transformer import (
    SelfAtten3DBlock,
)
...
class Generic_TransUNet(SegmentationNetwork):
    ...
    def __init__(
        ...
        """
        super(Generic_TransUNet, self).__init__()
        ...
        for u in range(num_pool):
            ...
            nfeatures_from_skip = (
                self.conv_blocks_context[-(2 + u)].output_channels
        ...
        self.features_num = int(
            np.prod(np.array(self.patch_size)) / np.prod(cum_upsample[0])
        )

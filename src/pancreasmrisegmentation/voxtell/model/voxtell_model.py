...
from pancreasmrisegmentation.voxtell.model.transformer import (
    TransformerDecoder,
...
class VoxTellModel(nn.Module):
    ...
    def __init__(
            ...
            [
                nn.Sequential(
                    ...
                    nn.Linear(
                        ...
                        decoder_config["channels"] * self.num_heads
                        if stage_idx != 0
                        else decoder_config["channels"],
                    ),
                )
        ...
        transformer_layer = TransformerDecoderLayer(
            d_model=query_dim, nhead=self.TRANSFORMER_NUM_HEADS, normalize_before=True
        )
    ...
    def forward(
        self, img: torch.Tensor, text_embedding: torch.Tensor = None
...
class VoxTellDecoder(nn.Module):
    ...
    def __init__(
        ...
        for stage_idx in range(1, n_stages_encoder):
            ...
            seg_layers.append(
                encoder.conv_op(
                    input_features_skip + num_heads, num_classes, 1, 1, 0, bias=True
                )
            )
    ...
    def forward(
        self, skips: List[torch.Tensor], mask_embeddings: List[torch.Tensor]
    ) -> List[torch.Tensor]:
        """
        ...
        for stage_idx in range(len(self.stages)):
            ...
            if stage_idx == (len(self.stages) - 1):
                ...
                fusion_features = torch.einsum(
                    "b c h w d, b n c -> b n h w d", x, mask_embedding_reshaped
                )

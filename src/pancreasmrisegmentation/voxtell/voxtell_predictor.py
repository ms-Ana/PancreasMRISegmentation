...
from pancreasmrisegmentation.voxtell.model.voxtell_model import VoxTellModel
from pancreasmrisegmentation.voxtell.utils.text_embedding import (
    last_token_pool,
...
class VoxTellPredictor:
    ...
    def __init__(
        ...
        quant_config = BitsAndBytesConfig(
            load_in_4bit=True,
            bnb_4bit_compute_dtype="float16",  # Recommended for performance
        ...
        arch_kwargs = plans["configurations"]["3d_fullres"]["architecture"][
            "arch_kwargs"
        ]
        self.patch_size = plans["configurations"]["3d_fullres"]["patch_size"]

        arch_kwargs = dict(**arch_kwargs)
        for required_import_key in plans["configurations"]["3d_fullres"][
            "architecture"
        ]["_kw_requires_import"]:
            if arch_kwargs[required_import_key] is not None:
                arch_kwargs[required_import_key] = pydoc.locate(
                    arch_kwargs[required_import_key]
                )
    ...
    def predict_sliding_window_return_logits(
        self, input_image: torch.Tensor, text_embeddings: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        if not isinstance(input_image, torch.Tensor):
            raise ValueError(
                f"input_image must be a torch.Tensor, got {type(input_image)}"
            )
    ...
    def _internal_predict_sliding_window_return_logits(
        ...
        def producer(data_tensor, slicer_list, queue):
            ...
            for slicer in slicer_list:
                patch = torch.clone(
                    data_tensor[slicer][None], memory_format=torch.contiguous_format
                ).to(self.device)
    ...
    def predict_single_image(
        self, data: np.ndarray, text_prompts: Union[str, List[str]]
    ) -> np.ndarray:
        """
        ...
        prediction = self.predict_sliding_window_return_logits(data, embeddings).to(
            "cpu"
        )
        ...
        segmentation_reverted_cropping = np.zeros(
            [prediction.shape[0], *orig_shape], dtype=np.uint8
        )

...
def _flatten_weights(target_dir):
    ...
    found_folds = [f for f in glob.glob(search_pattern, recursive=True) if os.path.isdir(f)]
...
@click.option(
    "--model",
    type=str,
    help="Which model weights to download, if not specified, no weights will be downloaded. Can specify multiple models by repeating the --model option. Can be one of " + ", ".join(WEIGHTS_SETTINGS.keys()) + ", or a custom model name if used with --link and --download-function options",
    required=False,
    multiple=True
)
@click.option(
...
def download_weights(weights_dir: str, redownload: bool, model: tuple[str], link: str = None, download_function: str = None, all: bool = False):
    """
    ...
    if all:
        ...
        for m in model:
            if m not in WEIGHTS_SETTINGS:
                raise ValueError(f"Model {m} not found in predefined weights settings. Please provide a valid model name or use --link and --download-function options to specify custom weights.")
    ...
    for name, settings in to_download.items():
        ...
        _download_weights_for_model(name, settings["link"], settings["download_func"], final_path)    
    
def _download_weights_for_model(model: str, link: str, download_function: str, final_path: str) -> tuple[str, str]:

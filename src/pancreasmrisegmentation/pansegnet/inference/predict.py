...
import nnunet
import pancreasmrisegmentation.pansegnet as pansegnet
...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    search_in = [join(nnunet.__path__[0], "training", "network_training"), 
                 join(pansegnet.__path__[0], "training", "network_training")]
    modules = ["nnunet.training.network_training", "pancreasmrisegmentation.pansegnet.training.network_training"]
    for search, module in zip(search_in, modules):
        
        tr = recursive_find_python_class([search], name, current_module=module)
        if tr is not None:
            break

...
def evaluate_segmentation_performance(pred_dir, gt_dir, subject_list=None, verbose=False):
	...
	for subj in subject_list:
		...
		if np.all(mask_pred == 0) or np.all(mask_pred == 1):
			if verbose:
				print(f"Subject {subj}: Prediction mask is uniform. Metrics set to 0.")			# GT non-empty but prediction empty or full of 1s→ complete miss.
...
@click.command()
@click.option("--pred_dir", type=str, required=True, help="Directory containing prediction files (.mha or .nii.gz)")
@click.option("--gt_dir", type=str, required=True, help="Directory containing ground truth files (.mha or .nii.gz)")
@click.option("--subject_list", type=str, default=None, help="Optional JSON file with {'subject_list': [...]}, or a comma-separated list of subject IDs")
@click.option("--save_path", type=str, default=None, help="Optional path to save the aggregated metrics as a JSON file")
...
def main(pred_dir: str, gt_dir: str, subject_list: str, save_path: str, verbose: bool):
	...
	results = evaluate_segmentation_performance(pred_dir, gt_dir,

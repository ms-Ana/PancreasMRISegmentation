...
def fast_resize_segmentation(segmentation, new_shape, mode="nearest"):
    ...
    if isinstance(segmentation, torch.Tensor):
        assert len(segmentation.shape[2:]) == len(new_shape), (
            f"segmentation.shape = {segmentation.shape}, new_shape = {new_shape}"
        )
    else:
        assert len(segmentation.shape[1:]) == len(new_shape), (
            f"segmentation.shape = {segmentation.shape}, new_shape = {new_shape}"
        )
    ...
    if mode == "nearest":
        ...
        reshaped = torch.zeros(
            [*seg_torch.shape[:2], *new_shape],
            dtype=seg_torch.dtype,
            device=seg_torch.device,
        )
        for i, c in enumerate(unique_labels):
            # mask = segmentation == c
            # reshaped_multihot = resize(mask.astype(float), new_shape, order, mode="edge", clip=True, anti_aliasing=False)
            mask = seg_torch == c
            reshaped_multihot = torch.nn.functional.interpolate(
                mask.float(), new_shape, mode=mode, align_corners=False
            )
...
def fast_resample_data_or_seg_to_shape(
    ...
    if is_seg:
        ...
        kwargs = {"mode": order_to_mode_map[order]}
    else:
        print(f"data.shape: {data.shape}")
        resize_fn = torch.nn.functional.interpolate
        kwargs = {"mode": order_to_mode_map[order], "align_corners": False}
    ...
    if np.any(shape != new_shape):
        ...
        assert reshaped_final_data.ndim == 4, (
            f"reshaped_final_data.shape = {reshaped_final_data.shape}"
        )

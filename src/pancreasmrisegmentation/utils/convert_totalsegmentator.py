...
def add_channel_dimension_to_folder(folder_path: str, file_ending: str = ".nii.gz"):
    for ppath in os.listdir(folder_path):
...
def combine_segmentation_to_file(
    segmentations: list[str], label_mapping: dict[str, int], output_path: str
):
...
@click.option(
    ...
    type=str,
    required=True,
    help="Path to the input folder containing the Totalsegmentator dataset.",
)
@click.option(
    "--output-folder",
    type=str,
    required=True,
    help="Path to the output folder  where the converted dataset will be saved.",
)
@click.option(
    "--meta-file",
    type=str,
    required=True,
    help="Path to the meta.csv file containing the metadata for the dataset.",
)
def convert_totalsegmentator_to_unet_dataset(
    ...
    label_mapping = {
        segmentation: idx + 1 for idx, segmentation in enumerate(list_of_segmentations)
    }
    meta_df = pd.read_csv(meta_file, sep=";")

    for info in tqdm(
        meta_df.itertuples(), total=len(meta_df), desc="Processing dataset"
    ):
        try:
            if info.split == "train":
                segmentation_output = os.path.join(
                    output_folder, "labelsTr", f"{info.image_id}.nii.gz"
                )
                combine_segmentation_to_file(
                    [
                        os.path.join(
                            input_folder, f"{info.image_id}/segmentations/{seg}.nii.gz"
                        )
                        for seg in list_of_segmentations
                    ],
                    label_mapping,
                    segmentation_output,
                )
                os.rename(
                    os.path.join(input_folder, f"{info.image_id}/mri.nii.gz"),
                    os.path.join(
                        output_folder, "imagesTr", f"{info.image_id}_0000.nii.gz"
                    ),
                )
            elif info.split == "test":
                segmentation_output = os.path.join(
                    output_folder, "labelsTs", f"{info.image_id}.nii.gz"
                )
                combine_segmentation_to_file(
                    [
                        os.path.join(
                ...
                os.rename(
                    os.path.join(input_folder, f"{info.image_id}/mri.nii.gz"),
                    os.path.join(
                        output_folder, "imagesTs", f"{info.image_id}_0000.nii.gz"
                    ),
                )
...
if __name__ == "__main__":
    ...
    add_channel_dimension_to_folder("/home/anastasiia/MasterThesis/external_data/Imaging/AMOS22/imagesTr", ".nii.gz")
